...
void LightTiler::tileIt(Spotlight* spot) {
	...
	btVector3 rotaxis = btVector3(0, 0, 1).cross(spot->dir);
	GLfloat angle = btVector3(0, 0, 1).dot(spot->dir);

	btVector3 up = btVector3(0, 1, 0).rotate(rotaxis, angle);
	btVector3 right = btVector3(1, 0, 0).rotate(rotaxis, angle);
	...
	U = btVector3(0, 0, 1).cross(spot->dir).cross(spot->dir);
	R = spot->dir.cross(U);
