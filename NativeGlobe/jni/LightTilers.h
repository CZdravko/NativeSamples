...
public:
	...
	void flatten(btVector3 *points, int count, btVector3 l, btVector3 ln, ConvexHull &ch);
	...
	void cutSphere(float Z, PointLight* p, btVector3 &RCut, float &rc);

	void project2Near(float &l, float Z);
	void project2Near(btVector3* point);

	void markZRowTilesSphere(btVector3* R, float r, int Z, int lightIndex);
	void markYRowTilesSphere(float from, float to, int count, int Z, int Y, int lightIndex);
	void flattenSphere(btVector3 *Bottom, btVector3* Top, float* rbottom, float* rtop);
